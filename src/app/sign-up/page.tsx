...
const JoinPage: React.FC = () => {
  const { register, handleSubmit, formState: { errors, isValid } } = useForm<JoinFormData>({
    mode: 'onChange'
  });

  const onSubmit: SubmitHandler<JoinFormData> = (data) => {
    console.log('계정 생성 시도', data);
    if (isValid) {
    ...
    <div className="flex justify-center items-center h-screen bg-gradient-to-br from-cyan-400 to-blue-500">
      <div className="bg-white bg-opacity-20 backdrop-blur-lg rounded-3xl shadow-xl overflow-hidden w-full max-w-md">
        <form className="p-8" onSubmit={handleSubmit(onSubmit)}>
          <h2 className="text-3xl mb-6 text-center font-bold text-white">Create Account</h2>
